...
\paragraph{}
Because each vehicle decides where to go by itself, sometimes two vehicles would choose to go into the same lane at the same time. To solve this problem, vehicles had to randomly be assigned priorities and Intersections were made in charge of this. The key was to make the Intersection a subscriber of the Ticker so it can read who is at the intersection at the same time the vehicles are deciding where to go. On ticker tick, the intersection calls the IntersectionController method "vehicleTurnFirst(...)", which takes the current intersection and a list of all vehicles at that intersection with priorities assigned randomly as parameters. Finally, the vehicle with the highest random priority is chosen to turn in the intersection. All other vehicles that have a green light have to wait for the vehicle with higher priority to turn. They will get another chance to turn on the next ticker tick. This process greatly improved the animation and simulation experience because vehicles no longer drive into each other at intersections.
